...
>
> There is a detailed knowledge base file at `src/gwtransport/fronttracking/KNOWLEDGE_BASE.md` describing the current implementation, modules, algorithms, and invariants.
>
> Please:
...
>    - **Meaningful and Explicit Tests**: Tests should not contain 1) conditional statements other than dealing with pytest parametrize or 2) `try/except` constructs unless absolutely necessary. Each comparison must directly serve the test purpose. Masked array comparisons are only allowed when the unmasked values are sufficient to validate the test purpose.
