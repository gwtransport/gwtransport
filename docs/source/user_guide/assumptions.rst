...
- **Choose** :mod:`gwtransport.diffusion_fast_fast` **when you want a fast approximate forward result and can tolerate a small error** (regularly-sampled monitoring data, exploratory or ensemble runs). It builds a single skewed Kreft-Zuber breakthrough on the native cumulative-volume grid, folding molecular diffusion (:math:`D_m`) in as an effective dispersivity :math:`\alpha_L + D_m r_{vpv}/(L\,\bar Q)` per streamtube (:math:`\bar Q` the record-mean flow), in one fast pass for *any* flow (constant or variable — no fallback). At **constant flow** it reproduces :mod:`gwtransport.diffusion_fast` to the interpolation floor (~1e-6 smooth, ~1e-4 sharp) for *every* regime, including heat (:math:`R > 1`, large :math:`D_m`). Under **variable flow** the microdispersion part stays exact and the molecular part carries a small commutator residual from the frozen :math:`\bar Q` — negligible when molecular diffusion is sub-dominant (:math:`\alpha_L > 0`, the usual case), growing to the multi-percent range only for sharp inputs in the molecular-diffusion-dominated regime (:math:`\alpha_L \approx 0`) under strongly variable flow. It is **approximate** — use :mod:`gwtransport.diffusion_fast` when you need machine precision, or for that molecular-dominated strongly-variable-flow corner. Its reverse (deconvolution) inverts the *same* approximate operator in banded form (banded Tikhonov, no dense matrix and no per-pore-volume closed-form loop), so it is much faster than :mod:`gwtransport.diffusion_fast`'s reverse and a forward-then-inverse round trip is self-consistent.
...
- :py:func:`~gwtransport.residence_time.full`, :py:func:`~gwtransport.residence_time.mean`, :py:func:`~gwtransport.residence_time.gamma`
- :py:func:`~gwtransport.deposition.deposition_to_extraction`, :py:func:`~gwtransport.deposition.extraction_to_deposition`
- :py:func:`~gwtransport.diffusion_fast.infiltration_to_extraction`
