...
which says "the residence time at extraction time :math:`t` is whatever :math:`\tau` it took to pump :math:`V_i \cdot R` cubic metres of water." This integral is solved by interpolation on the cumulative-flow curve and is exactly what :py:func:`gwtransport.residence_time.full` computes.
